...
function challenge(arr){
  let parts = []
  let syms = getSymbolPositions(arr)
  console.log(syms)
  arr.forEach((line, y) =>{
    ...
    // console.log('#', nums);
    nums?.forEach(n =>{
      let start = line.indexOf(n)
      let end = start + n.length -1
      let replacment = []
      replacment.length = n.length
      replacment = replacment.fill('0', 0).join('')
      // console.log('🦧',replacment);
      line = line.replace(n, replacment)
      // console.log('🦖',n , start, end);
      let isAdj = adjSymbol(syms, start, end, y)
      console.log(!!isAdj, n)
      if(isAdj && !parts.includes(n)) parts.push(parseInt(n))
    })
    console.log('>', line);
  })
  let sum = parts.reduce((acc, cur)=> acc + cur, 0)
  console.log('🌟', sum);
}

function adjSymbol(syms, start, end, partY){
    let adj = syms.find(s =>{
      if(partY -1 == s.y || partY == s.y || partY +1 == s.y){
        if( s.x >= start -1 && s.x <= end +1){
          console.log('next to symbol', s)
          return true
        }
      }
    })
...
function getSymbolPositions(arr){
  let out = []
  arr.forEach((line,y) =>{
...
function challenge2(arr){
  let answer = 0
  let gears = getGearPositions(arr)
  let numbers = getNumberPositions(arr)
  console.log(gears);
  console.log(numbers)
  gears.forEach(gear =>{
    const one = numbers.find(num => {
      if(gear.y <= num.y + 1 && gear.y >= num.y -1){
        if(gear.x >= num.start -1 && gear.x <= num.end +1){
          // console.log('⚙️',num.number, gear)
          return true
        }
      }
    })
    const two = numbers.find(num => {
    ...
    numbers = numbers.filter(n => n != one)
    numbers = numbers.filter(n => n != two)
    if(one && two){
      console.log(one.number, two.number)
      answer += one.number * two.number
    }
  })
  console.log('🌟', answer);
}

function getGearPositions(arr){
  let out = []
  arr.forEach((line, y) => {
    line.split("").forEach((char, x) => {
      if(char == '*') out.push({x,y})
    })
  })
  return out
}

/**@return {{start, end, y, number}[]} */
function getNumberPositions(arr){
  let out = []
  arr.forEach((line, y) => {
    let nums = line.match(numReg)
    nums?.forEach(number => {
      ...
      let replacment = []
      replacment.length = number.length
      replacment = replacment.fill('0', 0).join('')
      // console.log('🦧',replacment);
      line = line.replace(number, replacment)
      out.push({start, end, y, number})
    })
  })
  return out
}
