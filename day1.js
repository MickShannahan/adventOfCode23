...
let out = 0
let numReg = /zero|one|two|three|four|five|six|seven|eight|nine|[0-9]/ig
let revReg = /eno|owt|eerht|ruof|evif|xis|neves|thgie|enin|[0-9]/ig
// test2.forEach(line => {
//   let arr = line.split('')
//   let first = arr.find(el => (numReg).test(el))
//   let last = arr.reverse().find(el => (numReg).test(el))
//   first = convert(first)
//   last = convert(last)
//   console.log(first, last, parseInt(first + '' + last))
//   out += parseInt(first + ''+ last)
// })

// input.forEach(line =>{
//   let matches = line.match(numReg)
//   console.log(line)
...
input.forEach(line =>{
  console.log('✳️',line)
 let fMatches = line.match(numReg)
 let bMatches = line.split('').reverse().join('').match(revReg).map(s => s.split('').reverse().join(''))
 console.log(fMatches)
 console.log(bMatches)
 let f = convert(fMatches[0])
 let b = convert(bMatches[0])
 out += parseInt(f + b)
})
